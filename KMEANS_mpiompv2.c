 ...
 *
 */
	int* local_pointsPerClass;
	float* local_auxCentroids;

#	pragma omp parallel shared(local_classMap, local_data, centroids, auxCentroids, pointsPerClass, changes, maxDist) firstprivate(K, samples, local_sz, maxThreshold, minChanges, maxIterations)
	do
	{
		it++;
		changes = 0;
#		pragma omp for private(i, _class, minDist, k, dist)
...
#		pragma omp single
		...
		local_pointsPerClass = (int*) calloc(K,sizeof(int));
		local_auxCentroids = (float*) calloc(K*samples,sizeof(float));
